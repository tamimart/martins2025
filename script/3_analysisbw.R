...
# Funnel plot
# Plot and save 
png("figure/figure7bw.png", height = 450, width = 300, units = "mm", res = 600)
tiff("figure/figure7bw.tiff", height = 450, width = 300, units = 'mm', res = 150)
...
dfsubgroups <- dfsubgroups |> 
  ...
  mutate(tau2 = round(dfsubgroups$tau2, digits = 1),
         ...
         outline = 100,
         show_tau2 = ifelse(tau2 > 0, TRUE, FALSE)) 
...
generate_subgroup_plot_square <- function(dfsubgroups, pio_info, pio){
  ...
  incon_m <- dfsubgroups |>
    ...
    scale_fill_manual(values = c("inconsistency" = color_mice, "outline" = "black"), guide = "none") + #HERE and the next
    ggplot2::geom_point(
      aes(size = ifelse(tau2 > 0, tau2, NA) , y = 150),
      shape = 22,
      show.legend = TRUE,
      color = "black",
      fill = color_mice,
    ...
    scale_size_continuous(breaks = c(1, 10, 100), 
                          ...
                          limits = c(0, 100)) +
    theme(
      legend.position = c(0.08, 1.03),                      # Legenda no topo
...
generate_subgroup_plot_square(dfsubgroups, pio_info, pio = "population")
# Create plot to intervention - stratified 
generate_subgroup_plot_square(dfsubgroups, pio_info, pio = "intervention")
# Create plot to outcome - stratified 
generate_subgroup_plot_square(dfsubgroups, pio_info, pio = "outcome")
...
robplot <- df_rob_long |> 
  ...
  scale_fill_manual("SYRCLE", values = c("Low" = "grey80", "Unclear" = "grey50", "High" = "grey30"), guide = guide_legend(
    title.position = "top")) +
...
camaradesplot <- df_camarades_longo |> 
  ...
  geom_bar(position = "fill") + 
  scale_fill_manual("CAMARADES", values = c("Yes" = "grey80", "Incomplete" = "grey50", "No" = "grey30"), guide = guide_legend(
    title.position = "top")) +
  scale_y_continuous(labels = scales::percent) +
  scale_x_discrete(
    labels = function(x)
